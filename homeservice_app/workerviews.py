...
def view_stock(request):
    u=request.user
    print(u)
    data = seed.objects.filter(seeed=u)
    data1 = plant.objects.filter(plaant=u)
    data2 = fertilizer.objects.filter(fert=u)
...
def seedpage(request):
    ...
    if request.method == 'POST':
        ...
        if seed_form.is_valid():
            obj=seed_form.save(commit=False)
            obj.seeed=u
            obj.save()
...
def plantpage(request):
    ...
    if request.method == 'POST':
        ...
        if plant_form.is_valid():
            obj=plant_form.save(commit=False)
            obj.plaant=u
            obj.save()
            messages.info(request, 'product added')
            return redirect(worker_home)
...
def fertilizerpage(request):
    ...
    if request.method == 'POST':
        ...
        if fertilizer_form.is_valid():
            obj=fertilizer_form.save(commit=False)
            obj.fert=u
            obj.save()
            messages.info(request, 'product added')
            return redirect(worker_home)
